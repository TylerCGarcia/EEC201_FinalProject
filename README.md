...
Once the STFT of a signal is taken, mel-frequency wrapping is applied to the spectrum of the STFT. This is using a mel-spaced filter bank response, which can be seen below in Figure 5. The idea of behind these mel-spaced filter banks is that human perception of sound is only linear for up to around 1000 Hz, after which it follows a more logarithmic spacing scale. The mel-spaced filter banks follow this scaling. 
